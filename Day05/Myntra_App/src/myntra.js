...
// +++++++++++++++++ same code but in professional way ++++++++++++++++++++++
import React from "react";
import ReactDom from "react-dom/client";
import Card from "./component/Card";
import Header from "./component/Header";
...
// useState isliye use karte hain kyuki React re-render
// sirf tab karta hai jab state change hoti hai.

import { Greet as Goa, Meet as Moa } from "./utils/dummy";

function App() {

  // State bana rahe hain jisse UI control hoga
  // productList hi actual render hoga, products nahi.
  const [productList, setProductList] = useState(products);

  // Sorting Function (Immutable Way)
  // .sort() original array ko change karta hai ❌
  // Isliye pehle copy banate hain using spread operator ✅

  function sortArray() {
    const sorted = [...productList].sort((a, b) => a.Rs - b.Rs);
  ...
  // Taaki har baar fresh filtering ho.
  function priceAbove999() {
    const filtered = products.filter((value) => value.Rs > 999);
  ...
  return (
    <>
      ...
      {/* Body Section */}
      {/* Yaha state render ho rahi hai, original products nahi */}
      <div className="middle">
